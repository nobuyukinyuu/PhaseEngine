...
namespace PhaseEngine
{
    public class ImportBTI : VoiceBankImporter
    {
        ...
        public override IOErrorFlags Load(string path)
        {
            ...
            try
            {
                ...
                using (BinaryReader br = new BinaryReader(file.OpenRead()))
                {
                    ...
                    if (nameLen>0) v[0].name = System.Text.Encoding.ASCII.GetString(br.ReadBytes((int)nameLen));
                    ...
                    err |= LoadInstProperties(br, v);
                    bank[0] = v[0].ToJSONString();
                }  //Close file handle                
            }
        ...
        public static IOErrorFlags LoadInstProperties(BinaryReader br, Voice[] v, bool readAsBank = false)
        {
            try {
                var header = System.Text.Encoding.ASCII.GetString(br.ReadBytes(8));  //Read the instrument properties section header.
                ...
                {
                    var subsectionType = (InstProps) br.ReadByte();
                    var blocks = 1;  //Number of property blocks.  Always 1 if not a bank.
                    if (readAsBank)  blocks = br.ReadByte();

                    switch(subsectionType)  //Get the subsection identifier.
                    {
                        case InstProps.FMEnvelope:
                            ...
                            {
                                if (readAsBank) idx = br.ReadByte();
                                offset = (uint)(br.BaseStream.Position + br.ReadByte());
                                ...
                                v[idx].alg =  Algorithm.FromPreset(algorithm, Algorithm.PresetType.OPN);
                                v[idx].egs[0].feedback = feedback;
            ...
            } catch (PE_ImportException e) 
                { System.Diagnostics.Debug.Print(e.Message); return e.flags; }
