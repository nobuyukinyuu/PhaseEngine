...
public class Test2 : Label
{
    ...
    public Godot.Error PasteJSONData(string data, int target)
    {
        ...
        else if (o.HasItem("grid") || o.HasItem("connections"))
        {  //Probably an Algorithm

        }
        else if (o.HasItem("intent") && o.HasItem("envelope"))
    ...
    {  //Used by the UI to arrange operators in the kanban by stack.
    ...
    public float GetOutputScale(bool countMuted = true) //Used to score carriers by loudness, to provide the preview a scale factor
    {
        ...
        float Score(byte opNum)
        {
            ...
            {
                case OpBase.Intents.FM_OP:
                case OpBase.Intents.FM_HQ: //FM ops have a default score of 1. Stacking modulators only changes phase, not amplitude.
                    totalScore++;
                    break;
                case OpBase.Intents.FILTER: //Filters can have nothing connected to them so their score starts at 0 tops off at Filter.GAIN_MAX.
                    ...
                    {
                        case Filter.FilterType.PEAKING:
                        case Filter.FilterType.LOWSHELF:
                        case Filter.FilterType.HISHELF:
