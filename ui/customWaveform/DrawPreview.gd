...
func smooth():
	if !owner.get_node("H2/Smooth").pressed or owner.get_node("Amount").value==0:
		...
		var scale = owner.wavetable_size / 128.0  #Scales the distance to cull based on wavetable size.
		...
		if val > 0:  #Preserve edges
			...
			for i in dist:  #Limit dist to half of tbl size if increasing range of the val slider
				#This part tapers the edges
				var j = tbl.size()-1-i
				var percent = i/dist
				tmp[i] = lerp(smooth_tbl[i], tbl[i], percent)
				tmp[j] = lerp(smooth_tbl[j], tbl[j], percent)
			for i in tbl.size():
				var percent = ease(val/64.0, 0.25)  #This morphs the result into the smooth table
